...
module.exports = {
    ...
    getReport : asyncHandler( async (req , res , next) => {
        ...
        try {
            const data = await workBook.xlsx.writeFile( __dirname + `/invoice.xlsx`)
            .then(() => {
                res.send({
                status: "success",
    ...
    getSingleReport : asyncHandler( async (req , res , next) => {
        ...
        try {
            const data = await workBook.xlsx.writeFile( __dirname + `/singleInvoice.xlsx`)
            .then(() => {
                res.send({
                status: "success",
                message: "تم تجهيز التقرير بنجاح",
                path: `${path}/singleInvoice.xlsx`,
                });
            });
        }
        catch (err) {
